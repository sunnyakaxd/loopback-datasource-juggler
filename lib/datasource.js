...
function DataSource(name, settings, modelBuilder) {
  ...
  Object.keys(dao.prototype).forEach((name) => {
...
DataSource.prototype.connect = function(callback) {
  ...
  this.connector.connect((err, result) => {
    self.connecting = false;
...
DataSource.prototype.setup = function(name, settings) {
  ...
  if (name && !connector) {
    if (typeof name === 'object') {
      ...
      const result = DataSource._resolveConnector(name);
...
DataSource.prototype.defineRelations = function(modelClass, relations) {
  ...
  const createListener = function(name, relation, targetModel, throughModel) {
    if (!isModelDataSourceAttached(targetModel)) {
      targetModel.once('dataAccessConfigured', (model) => {
    ...
    if (throughModel && !isModelDataSourceAttached(throughModel)) {
      ...
      throughModel.once('dataAccessConfigured', (model) => {
        if (isModelDataSourceAttached(targetModel)) {
          // The target model is resolved
          const params = traverse(relation).clone();
          params.as = name;
  ...
  if (relations) {
    Object.keys(relations).forEach((rn) => {
      const r = relations[rn];
 ...
 */
DataSource.prototype.setupDataAccess = function(modelClass, settings, ignoreMixins) {
  if (this.connector) {
...
DataSource.prototype.attach = function(modelClass, noMix) {
  ...
  this.setupDataAccess(modelClass, modelClass.settings, noMix);
  modelClass.emit('dataSourceAttached', modelClass);
...
DataSource.prototype.automigrate = function(models, cb) {
  ...
  if (attachedModels && typeof attachedModels === 'object') {
    ...
    const invalidModels = models.filter(m => !(m in attachedModels));
...
DataSource.prototype.discoverSchema = function(modelName, options, cb) {
  ...
  this.discoverSchemas(modelName, options, (err, schemas) => {
    if (err || !schemas) {
      cb && cb(err, schemas);
      return;
    }
...
DataSource.prototype.discoverSchemas = function(modelName, options, cb) {
  ...
  const self = this;
  const dbType = this.connector.name || this.name;
  ...
  async.parallel(tasks, (err, results) => {
    ...
    if (Object.keys(otherTables).length === 0) {
      ...
      async.parallel(moreTasks, (err, results) => {
...
DataSource.prototype.discoverSchemasSync = function(modelName, options) {
  const self = this;
  const dbType = this.name || this.connector.name;
...
DataSource.prototype.disconnect = function disconnect(cb) {
  ...
  if (this.connected && (typeof this.connector.disconnect === 'function')) {
    this.connector.disconnect((err, result) => {
...
DataSource.prototype.transaction = function() {
  ...
  for (const i in dataSource.modelBuilder.models) {
